...
export default {
	search({query, startIndex = 0, order = string_constants.order.relevance}) {
		return GoogleBooksAPI().get(`/volumes?q=${stringToQueryParameter(query)}&orderBy=${order}&key=${import.meta.env.VITE_GOOGLE_BOOKS_API_KEY}&startIndex=${startIndex}&maxResults=16`)
	},
